...
describe("CLI Integration Tests", () => {
  ...
  test("should handle missing git repository", (t, done) => {
    ...
    child.on("close", (code) => {
      ...
      assert(
        stderr.includes("Not in a git repository") || stderr.includes("git"),
      );
      done();
    });
  });
  ...
  test("should save output to file", (t, done) => {
    ...
    const child = spawn(
      "node",
      ["bin/index.js", "--file", outputFile, "--engine", "local"],
      {
        stdio: "pipe",
      },
    );
...
describe("Git Integration Tests", () => {
  ...
  test("should detect staged changes", (t, done) => {
    ...
    const child = spawn(
      "node",
      ["bin/index.js", "--staged", "--engine", "local"],
      {
        stdio: "pipe",
      },
    );
  ...
  test("should work with specific commit hash", (t, done) => {
    ...
    try {
      ...
      if (commitHash) {
        const child = spawn(
          "node",
          ["bin/index.js", "--commit", commitHash, "--engine", "local"],
          {
            stdio: "pipe",
          },
        );
