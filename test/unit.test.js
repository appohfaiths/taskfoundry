...
import { loadConfig, getConfigSchema, validateConfig } from "../src/config.js";
...
describe("Configuration Tests", () => {
  test("should load default configuration", () => {
    ...
    // The effective default might be 'auto' or 'openai' depending on environment/system config
    // In clean test environment it should be 'auto'
  ...
  test("should validate configuration", () => {
    const invalidOutput = validateConfig({ engine: "auto", output: "invalid", temperature: 0.3, maxTokens: 1000 });
    assert.strictEqual(invalidOutput.isValid, false);
    assert(invalidOutput.errors.some(e => e.includes("Invalid output format")));

    const invalidEngine = validateConfig({ engine: "invalid", output: "markdown", temperature: 0.3, maxTokens: 1000 });
    assert.strictEqual(invalidEngine.isValid, false);
    assert(invalidEngine.errors.some(e => e.includes("Invalid engine")));

    const invalidTemp = validateConfig({ engine: "auto", output: "markdown", temperature: 3, maxTokens: 1000 });
    assert.strictEqual(invalidTemp.isValid, false);
    assert(invalidTemp.errors.some(e => e.includes("Invalid temperature")));
  });

  test("should return config schema", () => {
    const schema = getConfigSchema();

    assert(schema.properties.engine);
    assert(schema.properties.output);
    assert(schema.properties.temperature);
    assert.strictEqual(schema.properties.engine.type, "string");
    assert(schema.properties.engine.enum.includes("openai"));
    assert(schema.properties.engine.enum.includes("local"));
    assert(schema.properties.engine.enum.includes("groq"));
  });

  test("should handle project config file", () => {
    const configFile = TestHelper.createTempFile(
      ".taskfoundry.json",
      JSON.stringify({
        engine: "local",
        temperature: 0.5,
    ...
    assert(configFile.endsWith(".taskfoundry.json"));
...
describe("Formatter Tests", () => {
  ...
  test("should format markdown correctly", () => {
    ...
    assert(
      result.includes(
        "**Technical considerations**: Used bcrypt for password hashing",
      ),
    );
  });
  ...
  test("should format JSON correctly", () => {
    ...
    assert.strictEqual(
      parsed.technical_considerations,
      "Used bcrypt for password hashing and jsonwebtoken for token generation",
    );
  });
...
**Technical considerations**: Test tech notes`;
    ...
    const techMatch = mockResponse.match(
      /\*\*Technical considerations\*\*:([\s\S]*)/,
    );
...
describe("Error Handling Tests", () => {
  ...
  test("should handle malformed config files", () => {
    ...
    const config = loadConfig();
    assert.strictEqual(config.engine, "auto"); // Should use defaults
  });
});
