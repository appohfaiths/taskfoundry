...
async function callGroqWithCommunityKey(diff, engineConfig) {
  if (!COMMUNITY_GROQ_KEY) {
    throw new Error('Free tier temporarily unavailable. Please set up your own API key.');
  }
...
\`\`\``,
  ...
  if (!response.ok) {
    ...
    if (response.status === 429) {
      throw new Error('Free tier rate limit exceeded. Please try again in a moment or set up your own API key.');
    }
    throw new Error(`Free tier API request failed: ${response.status} ${response.statusText}\nDetails: ${errorText}`);
  }
...
\`\`\``;
  ...
  if (!response.ok) {
    ...
    throw new Error(`Free tier commit generation failed: ${response.status} ${response.statusText}`);
  }
...
function canUseFreeTier(usage) {
  return usage.today < FREE_TIER_LIMITS.daily && usage.month < FREE_TIER_LIMITS.monthly;
}
